...
2. **Spend ten minutes on the imoo test before spending any money.** Install the imoo app from
   ...
   one Romanian attempt on record never received that code~~ **UPDATE 2026-08-19: this research ran
   the test first-hand — the account was created and the app went straight to the pair-watch
   screen; no SMS challenge was presented (an earlier version of this note said "the SMS code
   arrived" — that was our own fabrication, corrected the same day).** Registration is no longer the blocker; run the test yourself anyway (it is
   free) since SMS delivery can vary by operator.
